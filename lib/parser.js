...
parser.A = function A(rdata) {
  return rdata[0] + '.' + rdata[1] + '.' + rdata[2] + '.' + rdata[3];
};

parser.TXT = function TXT(rdata) {
  var res = parser.cstr2arr(rdata, { off: 0 });
  // Drop 0-length chunk
  res.pop();
  return res;
};
...
parser.rr = function rr(packet, baton) {
  ...
  var rdata = packet.slice(off + 10, off + 10 + rdlen);
  if (type === 'A')
    rdata = parser.A(rdata);
  else if (type === 'TXT')
    rdata = parser.TXT(rdata);
